...
class RAF_student(data.Dataset):
    def __init__(self, split='Training', transform=None, noise=None):
        self.transform = transform
        self.split = split  # training set or test set

        try:
            self.data = h5py.File('/dev/shm/datasets/RAF_data_100.h5', 'r', driver='core')
        except:
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'Training':
            ...
            if self.noise != 'none':
                ...
                img_student_noise = self.student_norm(img_noise)
                return img_student_noise, target
            else:
