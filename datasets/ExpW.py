...
class ExpW_student(data.Dataset):
    def __init__(self, split='Training', transform=None):
        ...
        try:
            self.data = h5py.File('/dev/shm/datasets/ExpW_data_100.h5', 'r')
        except:
            self.data = h5py.File('datasets/ExpW_data_100.h5', 'r')
        ...
        if self.split == 'Training':
            ...
            self.test_data = self.test_data.reshape((5259, 100, 100, 3))
...
class MetaExpW(data.Dataset):
    def __init__(self, split='train', transform=None, student_norm=None, teacher_norm=None):
        self.transform = transform
        self.student_norm = student_norm
        self.teacher_norm = teacher_norm
        self.split = split
        self.data = h5py.File('datasets/ExpW_MetaData_100.h5', 'r', driver='core')
        ...
        if self.split == 'train':
            ...
            self.train_labels = self.data['train_data_label']
            self.train_data = np.asarray(self.train_data)
            self.train_data = self.train_data.reshape((15786, 100, 100, 3))
            ...
            self.valid_data = self.valid_data.reshape((5259, 100, 100, 3))
